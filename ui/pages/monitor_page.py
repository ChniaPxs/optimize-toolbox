...
class MonitorPage(BasePage):
    ...
    def _mobile_refresh_done(self, data):
        ...
        if data.get("ok"):
            ...
            mem_pct = min(99, 100 - int(100 * ma / mt)) if mt else 0  # 使用率
            lv = "?"
            try:
                lv = data["bat"][0]
            except Exception:
            ...
            # 手机模式下：主卡同步显示手机数据，评分区标记来源
            if getattr(self._app, "_is_mobile", False):
                try:
                    c1 = float(cpu1) if str(cpu1).replace(".", "", 1).isdigit() else 0.0
                    self._set_val("cpu", "%.1f%%" % c1, self._level_color(c1))
                    self._set_bar("cpu", min(1.0, c1 / 100.0), self._level_color(c1))
                    self._set_val("memory", "%d%%" % mem_pct, self._level_color(mem_pct))
                    self._set_bar("memory", mem_pct / 100.0, self._level_color(mem_pct))
                    try:
                        lv_i = int(lv)
                    except Exception:
                        lv_i = 0
                    self._set_val("battery", ("%d%%" % lv_i) if lv != "?" else "--", GREEN)
                    self._set_bar("battery", min(1.0, lv_i / 100.0), GREEN)
                    self._set_val("storage", str(total), ORANGE)
                except Exception:
                    pass
        else:
            st = data.get("state", "")
            if st == "unauthorized":
                ...
                # 同步顶栏真实连接状态，避免顶栏残留旧“已连接”
                if self._app and hasattr(self._app, "_chk_adb"):
                    try:
                        self._app._chk_adb()
                    except Exception:
                        pass
    ...
    def _set_score_text(self, text, color):
        if self._score_item:
            self.itemconfigure(self._score_item, text=text, fill=color)
    ...
    def _apply_optimize(self):
        ...
        if getattr(self._app, "_is_mobile", False):
            ...
            _msg = ("将根据当前监控结果执行安全优化：\n\n"
                    "· 清理用户与系统临时文件\n· 清理浏览器缓存\n· 刷新 DNS 缓存\n\n"
                    "均为安全缓存项，不会删除个人文件，继续？")
        if not messagebox.askyesno("执行优化", _msg, parent=self):
            return
    ...
    def _mobile_optimize_steps(self):
        """手机模式执行优化：手机垃圾快清 + 手机网络优化"""
        steps = []
        try:
            if self.adb:
                if self.adb.cls_log():
                    steps.append("手机日志缓冲已清空")
                if self.adb.cls_thumb():
                    steps.append("缩略图缓存已清空")
        except Exception:
            pass
        try:
            net = getattr(self._app, "net_optimizer", None)
            if net:
                ops, before, after = net._mobile_net_ops("game")
                if ops:
                    steps.extend(ops)
                    if before and before["avg"] and after and after["avg"]:
                        steps.append("延迟 %.0f→%.0fms" % (before["avg"], after["avg"]))
        except Exception:
            pass
        return steps
    ...
    def _optimize_worker(self):
        ...
        try:
            if getattr(self._app, "_is_mobile", False):
                steps.extend(self._mobile_optimize_steps())
                if not steps:
                    steps.append("手机未连接，跳过")
            else:
